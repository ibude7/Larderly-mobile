...
function InventoryCard({
  ...
  return (
    <Pressable
      ...
      className={`rounded-card border bg-surface dark:bg-[#171A21] p-4 ${selected ? 'border-primary' : 'border-line dark:border-[#303541]'} ${listMode ? 'mx-5 mb-3' : ''} overflow-hidden`}
    >
      ...
      <View className="flex-row justify-between items-start gap-2 pl-1">
        <View className="flex-1">
          <Text numberOfLines={2} className="text-base font-bold leading-tight text-ink dark:text-[#F6F1EA]">
            {item.name}
          </Text>
          {item.brand ? (
            <Text numberOfLines={1} className="text-xs font-semibold text-muted dark:text-[#9A948D] mt-0.5">
              {item.brand}
            </Text>
          ) : null}
        </View>
        ...
        <View className="h-12 w-12 items-center justify-center rounded-lg bg-canvas dark:bg-[#090A0D] overflow-hidden">
          {item.image_url ? (
            <Image
      ...
      <View className="mt-4 flex-row items-center justify-between border-t border-canvas dark:border-[#303541] pt-3 pl-1">
