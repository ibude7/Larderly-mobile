    ...
    <>
      <View className="mb-5 flex-row items-start justify-between gap-3">
        <View className="flex-1">
          <Text className="font-display text-4xl text-ink dark:text-ink-dark">Meal Planner</Text>
          <View className="mt-1 flex-row items-center gap-1.5">
            <Icon name="sparkles" size={14} color={c.primary} />
            <Text className="font-medium text-muted dark:text-muted-dark">AI meal planning</Text>
          </View>
        </View>
      ...
      <View className="gap-4">
        {weekDays.map((day, dayIndex) => {
          ...
          return (
            <View
              key={dateStr}
              className={`overflow-hidden rounded-card border bg-surface dark:bg-surface-dark ${
                isToday ? 'border-primary/40' : 'border-line dark:border-line-dark'
              }`}
            >
              <View
                className={`flex-row items-center justify-between border-b border-line dark:border-line-dark px-4 py-3 ${
                  isToday ? 'bg-primary/5' : 'bg-canvas dark:bg-canvas-dark'
                }`}
              >
                <View className="flex-row items-center gap-3">
                  <View
                    className={`h-10 w-10 items-center justify-center rounded-2xl ${
                      isToday ? 'bg-primary' : 'border border-line dark:border-line-dark bg-surface dark:bg-surface-dark'
                    }`}
                  >
                    <Text
                      className={`text-sm font-black ${isToday ? 'text-white' : 'text-ink dark:text-ink-dark'}`}
                    >
                      {day.getDate()}
                    </Text>
                  </View>
                  <View>
                    ...
                    <Text className="text-sm font-bold text-ink dark:text-ink-dark">{dayLabel}</Text>
                  </View>
                </View>
                {isToday ? (
              ...
              <View className="gap-0">
                {MEAL_TYPES.map((mealType) => {
                  ...
                  return (
                    <View
                      key={mealType}
                      className="flex-row items-center gap-3 border-b border-line dark:border-line-dark px-4 py-3 last:border-b-0"
                    >
                      ...
                      <View className="flex-1">
                        ...
                        {slotMeals.length > 0 ? (
                          <View className="mt-1 gap-1">
                            {slotMeals.map((meal) => (
                              <Pressable
                                ...
                                className="rounded-xl border border-line dark:border-line-dark bg-canvas dark:bg-canvas-dark px-2.5 py-1.5"
                              >
                          ...
                          <Text className="mt-0.5 text-xs text-muted dark:text-muted-dark">No meal planned</Text>
                        )}
                      </View>
                      <Pressable
                        onPress={() => setSelectedSlot({ date: dateStr, mealType })}
                        hitSlop={8}
                        className="h-9 w-9 items-center justify-center rounded-full border border-line dark:border-line-dark bg-surface dark:bg-surface-dark"
                      >
