...
export default function SecuritySection() {
  ...
  if (isAnonymous) {
    return (
      <View className="rounded-2xl border border-warning/30 bg-warning/10 p-4">
        <Text className="font-semibold text-ink dark:text-[#F0EEE9]">Security</Text>
  ...
  return (
    <View className="rounded-2xl border border-line dark:border-[#2A2A35] bg-surface dark:bg-[#1A1A22] p-4">
      <View className="mb-3 flex-row items-center justify-between">
        <Text className="font-semibold text-ink dark:text-[#F0EEE9]">Security</Text>
        <Text className="text-xs font-bold uppercase text-muted dark:text-[#6B6878]">{factors.length > 0 ? '2FA on' : '2FA off'}</Text>
      </View>
      ...
      {factors.length === 0 ? (
        <Text className="mb-3 text-sm text-muted dark:text-[#6B6878]">No second factor enrolled.</Text>
      ) : (
        factors.map((f) => (
          <View key={f.uid} className="mb-2 flex-row items-center justify-between rounded-xl bg-canvas dark:bg-[#0F0F13] px-3 py-2">
            <View>
              <Text className="font-medium text-ink dark:text-[#F0EEE9]">{f.displayName ?? 'Phone'}</Text>
              <Text className="text-xs text-muted dark:text-[#6B6878]">{f.phoneNumber ?? 'SMS factor'}</Text>
            </View>
      ...
      <Modal isOpen={open} onClose={() => setOpen(false)} title="Add SMS two-factor">
        {step === 'enter-phone' ? (
          ...
          <View className="gap-3">
            <Text className="text-sm text-muted dark:text-[#6B6878]">Enter the 6-digit code sent to {toE164(phoneNumber)}.</Text>
            <TextField
